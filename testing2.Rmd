...
for (i in 1:7){
  ...
  # write.table(mypred, paste0("pp/",data[i],"-processed.txt"), row.names = F, quote = F)
...
```{r}
library(ggpubr)
x1 <- pivot_longer(true_all[,13:14], everything(),names_to = "Features", values_to = "Value")
x2 <- pivot_longer(true_all[,c(15,18)], everything(),names_to = "Features", values_to = "Value")
x3 <- pivot_longer(true_all[,16:17], everything(),names_to = "Features", values_to = "Value")

p1 <- ggplot(x1, aes(x=Features, y=Value, fill=Features)) +
  geom_boxplot() +
  scale_fill_manual(values = c("seagreen","gold"))
p2 <- ggplot(x2, aes(x=Features, y=Value, fill=Features)) +
  geom_boxplot() +
  scale_fill_manual(values = c("blue","orange"))
p3 <- ggplot(x3, aes(x=Features, y=Value, fill=Features)) +
  geom_boxplot() 
...
count=1
for (i in seq(0.670,0.980, by=0.001)){
...
## prediction
```{r}
testing <- preprocess.2("output/snv-parse-real2_part2.csv")
write.table(testing, paste0("pp/","real2_part2","-processed.txt"), row.names = F, quote = F)
vcf.prob = predict(vcf.fit, testing, type="response")
testing = testing[,1:3]
