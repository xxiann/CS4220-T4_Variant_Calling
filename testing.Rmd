...
#               vs_SSC = ifelse(is.na(vs_SSC), med, vs_SSC),
#               MSI = ifelse(is.na(vd_MSI), 0, vd_MSI)) 
#   return(df)
# }

preprocess.2 <- function(file, count=2, mapq=40, p.value=0.05, p.v = 1, msi=1){
  df <- read.csv(file)
  
  ## obtaining VC
  x <- data.frame(t(data.frame(strsplit(df$REF_MFVdVs, "/"), row.names =c("CALL_mutect2","CALL_freebayes","CALL_vardict","CALL_varscan"))))
  rownames(x) <- c(1:nrow(x))
  x[!(is.na(x)|x=="NA")] = TRUE
  x[(is.na(x)|x=="NA")] = FALSE
...
for (i in 1:7){
  truth <- read.table(truthfiles[i]) %>% # bed files
      mutate(prediction = TRUE) # add an additional column
  mypred <- preprocess.df(vcffiles[i])
  # mypred <- read.csv(vcffiles[i])
...
## fitting model
# vcf.fit = glm(prediction ~ FILTER + MQ + vs_SPV + vd_SSF + MSI, data=full, family=binomial)
...
for (i in 1:7){
  ...
  called <- testing[vcf.prob > .939,1:3]
...
#     mutate(prediction = TRUE) # add an additional column
# mypred <- preprocess.df(vcffiles[7])
# testing <- left_join(mypred, truth, by = c("Chr"="V1", "START_POS_REF"="V2", "END_POS_REF"="V3")) %>%
#     mutate(prediction = ifelse(is.na(prediction), FALSE, TRUE))
# vcf.prob = predict(vcf.fit, testing, type="response")
# 
# count=1
# for (i in seq(0.930,0.951, by=0.001)){
...
saveRDS(vcf.fit, "model_file.rds")
```
...
model1 = randomForest(x = full[,9:18],
                      y = full$prediction,
                      ntree = 500)
...
y_pred = predict(model1, newdata = testing[,9:18])
...
```{r}
model2 = randomForest(x = full[,15:20],
                      y = full$prediction,
                      ntree = 500)
...
# Predicting the Test set results
y_pred = predict(model2, newdata = testing[,15:20])
