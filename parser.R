...
parse.snv = function(sample.dir) {
  ...
  meta_data$Mutect2 <- meta_data$REF_Mutect2 == meta_data$REF
  meta_data$Mutect2[is.na(meta_data$Mutect2)] <- FALSE
  meta_data$Freebayes <- meta_data$REF_Freebayes == meta_data$REF
  meta_data$Freebayes[is.na(meta_data$Freebayes)] <- FALSE
  meta_data$Vardict <- meta_data$REF_Vardict == meta_data$REF
  meta_data$Vardict[is.na(meta_data$Vardict)] <- FALSE
  meta_data$Varscan <- meta_data$REF_Varscan == meta_data$REF
  meta_data$Varscan[is.na(meta_data$Varscan)] <- FALSE
  ...
  parse_snv <- meta_data[,c("seqnames","start","end","REF","ALT","Mutect2","Freebayes","Vardict","Varscan","Sample_Name",
                            "FILTER_Mutect2","FILTER_Freebayes","FILTER_Vardict","FILTER_Varscan",
                            names.features)]
  colnames(parse_snv) <- c("Chr","START_POS_REF","END_POS_REF","REF","ALT","Mutect2","Freebayes","Vardict","Varscan",
                           "Sample_Name","FILTER_Mutect2","FILTER_Freebayes","FILTER_Vardict","FILTER_Varscan",
                           feature.cols)
